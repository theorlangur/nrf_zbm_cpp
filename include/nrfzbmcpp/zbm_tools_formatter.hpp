#ifndef ZBM_TOOLS_FORMATTER_HPP_
#define ZBM_TOOLS_FORMATTER_HPP_
#include <string_view>
#include <span>
#include <expected>
#include <cstring>
...
namespace zbm
{
    namespace tools
    {
        ...
        {
            constexpr static F posExpThreshold=F(1e7);
            constexpr static F negExpThreshold=F(1e-5);
        };
            ...
            static int16_t normalizeFloat(T &value)
            {
                ...
                if (value >= FP::posExpThreshold) {
                    if (double(value) >= 1e256) {
                        value = float(double(value) / 1e256);
                        exponent += 256;
                    }
                    if ((double)value >= 1e128) {
                        value = float(double(value) / 1e128);
                        exponent += 128;
                    }
                    if ((double)value >= 1e64) {
                        value = float(double(value) / 1e64);
                        exponent += 64;
                    }
                    if ((double)value >= 1e32) {
                        value = float(double(value) / 1e32);
                        exponent += 32;
                    }
                    if ((double)value >= 1e16) {
                        value = float(double(value) / 1e16);
                        exponent += 16;
                    }
                    if ((double)value >= 1e8) {
                        value = float(double(value) / 1e8);
                        exponent += 8;
                    }
                    if ((double)value >= 1e4) {
                        value = float(double(value) / 1e4);
                        exponent += 4;
                    }
                    if ((double)value >= 1e2) {
                        value = float(double(value) / 1e2);
                        exponent += 2;
                    }
                    if ((double)value >= 1e1) {
                        value = float(double(value) / 1e1);
                        exponent += 1;
                    }
        ...
        template<size_t N>
        struct formatter_t<std::array<uint8_t, N>>
        {
            template<FormatDestination Dest>
            static std::expected<size_t, FormatError> format_to(Dest &&dst, std::string_view const& fmtStr, const std::array<uint8_t, N> &v)
            {
                for(uint8_t b : v)
                {
                    uint8_t hi = (b >> 4) & 0xf;
